...
def get_connected_devices_by_class(device_type=""):
    try:
        if device_type == "":
            return subprocess.check_output(f'powershell.exe Get-PnpDevice -Status \"OK\" | Select-Object -Property Class, FriendlyName, InstanceID', text=True, errors="replace").strip()
        else:
            return subprocess.check_output(f'powershell.exe Get-PnpDevice -Class \"{device_type}\" -Status \"OK\" | Select-Object -Property FriendlyName, InstanceID', text=True, errors="replace").strip()
